...
[![Live Demo](https://img.shields.io/badge/%F0%9F%8C%90%20Live%20Demo-navicache--x-b8f83d)](https://hellozicky.github.io/NaviCache/)
...
<p align="center">
  <a href="https://hellozicky.github.io/NaviCache/">
    <img src="assets/demo/navicache_x_demo.gif" alt="NaviCache animated demo — trajectory tracking, the skip/update decision engine, and speed-quality results" width="880">
  </a>
  <br>
  <em>▶&nbsp; Click the animation to open the <a href="https://hellozicky.github.io/NaviCache/">interactive demo</a> (chapter navigation, pause/play).</em>
</p>
...
- **[August 2026]** 🌐 The interactive demo page is live: [hellozicky.github.io/NaviCache](https://hellozicky.github.io/NaviCache/).

## 🌐 Interactive Demo

An animated, data-driven walkthrough of NaviCache is available at **[hellozicky.github.io/NaviCache](https://hellozicky.github.io/NaviCache/)** (source in [`docs/`](docs/)). It loops through three chapters:

| Chapter | What it shows |
|---|---|
| `01 · TRAJECTORY` | The input-output feature-change trajectory (ΔI → ΔO) drifts during denoising. Offline fitting (TeaCache) and zero-order hold (EasyCache) are compared against NaviCache's self-calibrated tracking on a real 29-step Wan2.1 trace, with per-method MAE. |
| `02 · DECISION` | The dual-state estimator in action: initial alignment → prior projection → uncertainty-aware safety gate, with a live compute / skip / update decision trace over all denoising steps. |
| `03 · RESULT` | A high-motion Wan2.1 I2V case (TeaCache vs. NaviCache) plus speedups across the three supported backbones. |

The animation replays the real measured trajectory shipped in [`docs/assets/merged_stats_diff.csv`](docs/assets/merged_stats_diff.csv). A pre-rendered clip is also available as [GIF](assets/demo/navicache_x_demo.gif) / [MP4](assets/demo/navicache_x_demo.mp4).
...
│   └── demo/                      # Pre-rendered demo animation (GIF/MP4)
├── docs/                          # Interactive demo page (served via GitHub Pages)
